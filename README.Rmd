...
**ggcube** extends ggplot2 into three dimensions, providing a grammar of graphics for 3D data visualization. Create 3D scatter plots, surfaces, volumes, and complex layered visualizations using familiar ggplot2 syntax with `aes(x, y, z)` and `coord_3d()`.

The package provides a variety of 3D-specific layer functions to render surfaces, prisms, points, and paths in 3D. Users can control plot geometry with 3D projection parameters, can apply a range of 3D lighting models, and can mix 3D layers with 2D layers rendered on cube faces.
...
- `stat_surface_3d()` renders surfaces based on existing grids data such as terrain data
