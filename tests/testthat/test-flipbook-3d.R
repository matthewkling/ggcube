# Tests for flipbook_3d front-end logic: axis assignment, validation, grid
...
# --- resolve_flipbook_axes: classification and axis assignment ---------------

test_that("a single ranged parameter yields a 1D flipbook on that axis", {
      expect_equal(resolve_flipbook_axes(yaw = c(0, 360))$axes, "yaw")
      expect_equal(resolve_flipbook_axes(roll = c(0, 360))$axes, "roll")
      expect_equal(resolve_flipbook_axes(pitch = c(0, 90))$axes, "pitch")
})

test_that("two ranges assign x and y by priority", {
      # yaw is top x priority; roll is top y priority.
      expect_equal(resolve_flipbook_axes(yaw = c(0, 360), roll = c(-45, 45))$axes,
                   c("yaw", "roll"))
      # yaw on x, pitch falls to y.
      expect_equal(resolve_flipbook_axes(yaw = c(0, 360), pitch = c(0, 90))$axes,
                   c("yaw", "pitch"))
      # no yaw: pitch takes x (next x priority), roll takes y.
      expect_equal(resolve_flipbook_axes(pitch = c(0, 90), roll = c(-45, 45))$axes,
                   c("pitch", "roll"))
})

test_that("fixed parameters are recorded and not treated as axes", {
      spec <- resolve_flipbook_axes(yaw = c(0, 360), pitch = 15, roll = c(-45, 45))
      expect_equal(spec$axes, c("yaw", "roll"))
      expect_equal(spec$fixed$pitch, 15)
})

test_that("resolve_flipbook_axes errors on invalid range counts", {
      # No ranges at all.
      expect_error(resolve_flipbook_axes(pitch = 5),
                   "at least one rotation parameter")
      # A length-3 parameter is not a valid range.
      expect_error(resolve_flipbook_axes(yaw = c(0, 180, 360)),
                   "length 3")
      # Three ranged parameters.
      expect_error(
            resolve_flipbook_axes(yaw = c(0, 360), pitch = c(0, 90),
                                  roll = c(-1, 1)),
            "at most two")
})
...
test_that("1D grid has the requested number of rows and holds others constant", {
      spec <- resolve_flipbook_axes(yaw = c(0, 90))
...
test_that("2D grid is the row-major outer product with x varying fastest", {
      spec <- resolve_flipbook_axes(yaw = c(0, 100), roll = c(-10, 10))
...
test_that("fixed parameter overrides the coord default in the grid", {
      spec <- resolve_flipbook_axes(yaw = c(0, 90), pitch = 42)
...
test_that("a full-turn axis loops and drops its duplicate endpoint", {
      spec <- resolve_flipbook_axes(yaw = c(0, 360))
...
test_that("a partial-sweep axis clamps and samples inclusively", {
      spec <- resolve_flipbook_axes(roll = c(-90, 90))
...
test_that("per-axis auto-detect mixes loop and clamp in 2D", {
      ...
      spec <- resolve_flipbook_axes(yaw = c(0, 360), roll = c(-45, 45))
...
test_that("a bare scalar start is accepted for a 1D flipbook", {
      spec <- resolve_flipbook_axes(yaw = c(0, 350))
      grid <- build_param_grid(fake_coord(), spec, n = 36, loop = TRUE,
                               start = 30)
      expect_equal(grid$start, 3L)
...
test_that("an unnamed swept axis in start falls back to its midpoint", {
      spec <- resolve_flipbook_axes(yaw = c(0, 100), roll = c(-90, 90))
...
test_that("a bare scalar start errors for a 2D flipbook", {
      spec <- resolve_flipbook_axes(yaw = c(0, 100), roll = c(-90, 90))
      expect_error(
